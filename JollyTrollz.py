...
from JollyTrollz_about import *
from JollyTrollz_strings import *
from JollyTrollz_seccomp import *
from JollyTrollz_syscall_table import *
from JollyTrollz_jtdisasm import *
from JollyTrollz_vffuzz import *
from JollyTrollz_jollydbg import *
...
class JollyTrollz(QtWidgets.QMainWindow, mainwindow.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.actionVuln_function_scanner.triggered.connect(self.vffuzz_func)
        self.actionDBG.triggered.connect(self.jollydbg_func)

        self.filename = ''
        self.barray = bytearray()
    
        self.default_os_abi = ["No extensions or unspecified", "Hewlett-Packard HP-UX", "NetBSD", "GNU", "<unknown: 4>", "<unknown: 5>", "Sun Solaris", "AIX", "IRIX", "FreeBSD", "Compaq TRU64 UNIX", "Novell Modesto", "Open BSD", "Open VMS", "Hewlett-Packard Non-Stop Kernel", "Amiga Research OS", "The FenixOS highly scalable multi-core OS", "Nuxi CloudABI", "Stratus Technologies OpenVOS"]
    
        self.default_type = ["NONE/No file type", "REL/Relocatable file", "EXEC/Executable file", "DYN/Shared object file", "CORE/Core file", "LOOS/Operating system-specific", "HIOS/Operating system-specific", "LOPROC/Processor-specific", "HIPROC/Processor-specific"]
    ...
    def vffuzz_func(self):
        if(len(self.filename) != 0):
            os.system("strings " + self.filename + " > vffuzz_strings.txt")
            os.system("objdump -M intel -d " + self.filename + " > vffuzz_objdump.txt")
            os.system("python3 ROPgadget.py --binary " + self.filename + " > vffuzz_gadgets.txt")
            self.vvfzz = VffuzzApp()
            self.vvfzz.show()
    
    def jollydbg_func(self):
        if(len(self.filename) != 0):
            os.system("objdump -M intel -d " + self.filename + " > jdbg_obj_dump.txt")
            r = r2pipe.open(self.filename)
            r.cmd('aaa')
            r.cmd('s main; agf > main_graph.txt')
            r.cmd('afl > functions.txt')
            r.cmd('quit')
            f = open("save_filename.txt", 'w')
            f.write(self.filename)
            f.close()
            self.jdbg = JollyDBGApp()
            self.jdbg.show()